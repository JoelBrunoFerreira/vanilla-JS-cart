...
function checkout() {
  let paypalFormHTML = `
      ...
      <input type="hidden" name="business" value="j.bruno.t.ferreira@gmail.com">
      `;
...
function countCarTotal() {
  let cartotal = 0;
  cart.forEach(cartItem => {
    cartotal += cartItem.quantity * cartItem.price;
  });
  document.querySelector('[data-action="CHECKOUT"]').innerText = `Pay $${cartotal}`;
}
